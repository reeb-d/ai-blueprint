...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        case 'search_paradigms': {
            ...
            const results = paradigmsData.filter((p) => p.name.toLowerCase().includes(query) ||
        ...
        case 'search_ux_laws': {
            const query = args.query.toLowerCase();
            const results = uxLawsData.filter((law) => law.name.toLowerCase().includes(query) ||
        ...
        case 'get_design_recommendations': {
            ...
            if (useCase.toLowerCase().includes('chat') || useCase.toLowerCase().includes('conversation')) {
                ...
                if (paradigm)
                    recommendations.recommended_paradigms.push(paradigm);
            }
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    ...
    switch (name) {
        case 'design_review': {
            const designDescription = args?.design_description || '';
            return {
                messages: [
                    {
                        role: 'user',
